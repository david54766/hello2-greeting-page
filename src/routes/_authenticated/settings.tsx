...
function Settings() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq("user_id", user.id)
      .maybeSingle()
      .then(({ data }) => setDeletionRequested(Boolean(data)));
  }, [user]);
  ...
  const requestAccountDeletion = async () => {
    ...
    toast.success("Account deletion requested. We will complete the request within 30 days.");
  };
    ...
    <div className="mx-auto max-w-3xl px-6 py-12">
      ...
      <section className="mt-10">
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          Tier upgrades are managed by the platform owner during launch. Stripe self-serve billing
          wires up next.
        </p>
      </section>
      ...
      <section className="mt-10">
        <h2 className="font-display text-2xl">Privacy and legal</h2>
        ...
        <div className="mt-8 rounded-xl border border-destructive/30 p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Request permanent deletion of your account and associated personal data. Requests are
            completed within 30 days.
          </p>
          <Button
            type="button"
            variant="outline"
            className="mt-4 rounded-full border-destructive/50 text-destructive"
            disabled={deletionRequested || requestingDeletion}
    ...
    <section className="mt-10">
      ...
      <div className="mt-6 space-y-3">
        ...
        {!loading && centers.length === 0 && (
          <p className="text-sm text-muted-foreground">No centers yet. Add your first below.</p>
        )}
        {centers.map((c) => (
          <div
            key={c.id}
          ...
          >
            <div className="min-w-0">
              ...
              <div className="mt-1 text-sm text-muted-foreground">
                {[c.city, c.state].filter(Boolean).join(", ") || "—"} · {c.enrollment_size ?? "?"}/
                {c.capacity ?? "?"} enrolled · {c.tuition_range ?? "tuition n/a"} ·{" "}
                {c.staff_count ?? "?"} staff
